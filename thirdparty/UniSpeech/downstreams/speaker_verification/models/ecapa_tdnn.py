    ...
    def __init__(self, feat_dim=80, channels=512, emb_dim=192, global_context_att=False,
        ...
        if feat_type == 'fbank':
            ...
            if config_path is None:
                ...
                # print(f"Loading feature extractor from s3prl hub: {feat_type}")
                # self.feature_extract = torch.hub.load('s3prl/s3prl', feat_type)
                from s3prl.upstream.wavlm.expert import UpstreamExpert
                print(f"Loading feature extractor from local: {feat_type}")
                self.feature_extract = UpstreamExpert(feat_type + '.pt')
            else:
