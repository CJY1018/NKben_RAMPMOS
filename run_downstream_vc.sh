...
for LANG in "${LANGUAGES[@]}"; do
  echo "正在评估 $MODEL_NAME 模型（$LANG）..."
  python run_all_sslmos.py --meta_csv "InputData_VC/${LANG}/meta_$MODEL_NAME.csv" --lang "$LANG"
done
...
for LANG in "${LANGUAGES[@]}"; do
  echo "正在评估 $MODEL_NAME 模型（$LANG）..."
  python run_all_audiobox.py --meta_csv "InputData_VC/${LANG}/meta_$MODEL_NAME.csv" --lang "$LANG"
done
...
for LANG in "${LANGUAGES[@]}"; do
  echo "正在评估 $MODEL_NAME 模型（$LANG）..."
  python run_all_utmos.py --meta_csv "InputData_VC/${LANG}/meta_$MODEL_NAME.csv" --lang "$LANG"
done
